...
1. 双击 GitHub Release 中的安装程序，点击“选择目录”，选择包含 `FFmpegFreeUI.exe` 的 3FUI 根目录，然后点击“安装”。安装后的目录结构为：
   ```
...
插件页面加载后会在后台向 GitHub `maxzrb/VideoEnhancer` 的 `releases/latest` 检查更新，底部也可手动点击“检查更新”。发现新版本时，用户确认后下载并校验更新包。

模型下载页会显示 `Plugin/videoenhancer.exe` 资源，但“下载全部”会排除当前插件 EXE；正式升级使用底部“检查更新”入口。插件本体不混入模型解压目录。

插件下载并校验更新 EXE 后，等待 3FUI 退出，再替换运行 EXE 与插件 DLL，失败时回滚并重启 3FUI。`aria2-next`、模型和配置会保留。下载首选 ModelScope 镜像，失败回退 GitHub Release 资产（`VIDEOENHANCER_UPDATE_DATASET=owner/name` 可覆盖）；版本检查首选 GitHub、失败时使用 ModelScope，检查仓库可用 `VIDEOENHANCER_UPDATE_GITHUB_REPO=owner/name` 覆盖；配置中的 `AutoCheckUpdates` 可关闭启动后台检查。
