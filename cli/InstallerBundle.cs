...
/// <summary>
/// 安装器容器：可执行文件前缀是纯净的 videoenhancer.exe，尾部只承载安装时释放的独立组件。
/// Windows 加载器会忽略 PE 尾部数据；运行程序则通过固定尾标定位并校验清单。
/// </summary>
    ...
    internal static void Create(string runtimePath, string outputPath, string payloadRoot)
    {
        ...
        try
        {
            ...
            using (var destination = new FileStream(temporary, FileMode.CreateNew, FileAccess.Write, FileShare.None))
            {
                ...
                {
                    bundleManifest.RuntimeSha256 = CopyAndHash(source, destination, source.Length);
