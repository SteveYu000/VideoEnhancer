...
class UserModelImportContractTests(unittest.TestCase):
    ...
    def test_segmented_page_and_shell_per_item_delete_are_wired(self):
        ...
        self.assertIn("If File.Exists(ConfigPath) Then", config)
        self.assertNotIn("GetLegacyConfigPath", config)
        self.assertNotIn("loadedLegacyConfig", config)
        self.assertNotIn("SpecialFolder.LocalApplicationData", config)
